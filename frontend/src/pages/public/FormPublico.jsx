...
export default function FormPublico() {
  ...
  useEffect(() => {
    PublicoService.listarCursos()
      .then((r) => setCursos(r.data))
      .catch(() => {});
    PublicoService.listarCiudades()
  ...
  useEffect(() => {
    ...
    const nombre = [a.nombre1, a.nombre2, a.apellido1, a.apellido2]
      .filter(Boolean)
      .join(" ");
    ...
    aspirantes[0]?.nombre1,
    aspirantes[0]?.nombre2,
    aspirantes[0]?.apellido1,
    aspirantes[0]?.apellido2,
    aspirantes[0]?.numero_documento,
    aspirantes[0]?.email,
    aspirantes[0]?.telefono,
    tipoEntidad,
  ]);
  ...
  const addAsp = () => {
    setAspirantes((p) => [...p, { ...ASP_VACIO }]);
    setPdfs((p) => [...p, null]);
  };
  const removeAsp = (idx) => {
    setAspirantes((p) => p.filter((_, i) => i !== idx));
    setPdfs((p) => p.filter((_, i) => i !== idx));
  };
  const setPdf = (idx, file) =>
    setPdfs((p) => p.map((v, i) => (i === idx ? file : v)));
  ...
  async function handleSubmit(e) {
    ...
    for (let i = 0; i < aspirantes.length; i++) {
      ...
      const n = i + 1;
      if (!a.nombre1?.trim()) {
        setError(`Aspirante ${n}: el primer nombre es obligatorio`);
        return;
      }
      if (!a.apellido1?.trim()) {
        setError(`Aspirante ${n}: el primer apellido es obligatorio`);
        return;
      }
      if (!a.numero_documento?.trim()) {
        setError(`Aspirante ${n}: el número de documento es obligatorio`);
        return;
      }
      if (!a.email?.includes("@")) {
        setError(`Aspirante ${n}: el correo electrónico es inválido`);
        return;
      }
      if (!a.telefono?.trim()) {
        setError(`Aspirante ${n}: el teléfono es obligatorio`);
        return;
      }
      if (!a.fecha_nacimiento) {
        setError(`Aspirante ${n}: la fecha de nacimiento es obligatoria`);
        return;
      }
      if (!a.laboral?.nivel_academico) {
        setError(`Aspirante ${n}: el nivel educativo es obligatorio`);
        return;
      }
      if (!a.contacto?.nombre?.trim()) {
        setError(
          `Aspirante ${n}: el nombre del contacto de emergencia es obligatorio`,
        );
        return;
      }
      if (!a.contacto?.telefono?.trim()) {
        setError(
          `Aspirante ${n}: el teléfono del contacto de emergencia es obligatorio`,
        );
        return;
      }
      if (!pdfs[i]) {
        setError(`Aspirante ${n}: el documento PDF es obligatorio`);
        return;
      }
    }
    setEnviando(true);
    try {
      ...
      pdfs.forEach((pdf, i) => {
        if (pdf) formData.append(`pdf_${i}`, pdf);
      });
  ...
  function resetFormulario() {
    ...
    setPdfs([null]);
    setCursoId("");
    setEmpresa(EMP_VACIO);
    setError("");
  }

  const headingStep1 = {
    title: "Registro de Solicitudes TSA",
    sub: "Seleccione su tipo de perfil para comenzar",
  };
    ...
    <div className="fp-shell">
      ...
      <div className="fp-body">
        <div className="fp-steps">
          ...
          <div
            className={
              "fp-step-dot" + (step === 2 ? " active" : step > 2 ? " done" : "")
            }
          />
        ...
        <div className="fp-heading">
          ...
          <p>
            {step === 1
              ? headingStep1.sub
              : step === 2
                ? "Complete la información del perfil seleccionado."
                : "Complete la información del aspirante asociada al perfil seleccionado."}
          </p>
          {step > 1 && (
            <div className="fp-perfil-pill">
              Perfil:{" "}
              <strong className="fp-label-tipo">
                {TIPO_LABELS[tipoEntidad]}
              </strong>
            </div>
          )}
        </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!empresa.nombre?.trim()) {
                    setError("El nombre de la empresa/entidad es obligatorio");
                    return;
                  }
                  if (!empresa.nombre_contacto?.trim()) {
                    setError("El nombre del solicitante es obligatorio");
                    return;
                  }
                  if (!empresa.nit?.trim()) {
                    setError("El NIT / número de documento es obligatorio");
                    return;
                  }
                  if (!empresa.email?.includes("@")) {
                    setError(
                      "El correo electrónico del solicitante es inválido",
                    );
                    return;
                  }
        ...
        {step === 3 && (
          <form onSubmit={handleSubmit} className="fp-form">
            <div className="fp-card">
              <div className="fp-card-header-row">
                <div className="fp-card-title fp-card-title--inline">
                  Aspirante{tipoEntidad !== "persona" ? "s" : ""} (
                  {aspirantes.length})
                </div>
              ...
              <div className="fp-asp-wrap">
                {aspirantes.map((asp, idx) => (
                  <AspCard
                    ...
                    onRemove={removeAsp}
                    canRemove={
                      tipoEntidad !== "persona" && aspirantes.length > 1
                    }
                    pdfFile={pdfs[idx]}
                    onPdf={setPdf}
                  />
            ...
            <div className="fp-consent">
              <strong>Ley 1581/2012 – Habeas Data:</strong> Los datos personales
              recopilados serán tratados exclusivamente por el SENA Sede
              Industrial Palmira con fines de gestión formativa. Al enviar este
              formulario autoriza el tratamiento de sus datos conforme a la
